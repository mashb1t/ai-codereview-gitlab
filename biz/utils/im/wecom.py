...
class WeComNotifier:
    ...
    def send_message(self, content, msg_type='text', title=None, is_at_all=False, project_name=None,
        ...
        try:
            ...
            if content_length <= MAX_CONTENT_BYTES:
                ...
                # 内容超过限制，需要分割发送
                logger.warning(_("消息内容超过 {MAX_CONTENT_BYTES} 字节限制，将分割发送。总长度: {content_length} 字节").format(
                    MAX_CONTENT_BYTES=MAX_CONTENT_BYTES, content_length=content_length))
                self._send_message_in_chunks(content, title, post_url, msg_type, is_at_all, MAX_CONTENT_BYTES)

        except Exception as e:
            logger.error(_("企业微信消息发送失败! {e}").format(e=e))
    ...
    def _send_message_in_chunks(self, content, title, post_url, msg_type, is_at_all, max_bytes):
        ...
        for i, chunk in enumerate(chunks):
            chunk_title = _("{title} (第{}/{}部分)").format(i + 1, len(chunks), title=title) if title else _("消息 (第{}/{)}部分)").format(i + 1, len(chunks))
    ...
    def _send_message(self, post_url, data, chunk_num=None, total_chunks=None):
        ...
        try:
            logger.debug(
                _("发送企业微信消息{} {chunk_num}/{total_chunks}: url={post_url}, data={data}").format(
                    _('分块') if chunk_num else '', chunk_num=chunk_num, total_chunks=total_chunks if chunk_num else '', post_url=post_url,
                    data=data))
            response = self._send_request(post_url, data)

            if response and response.get('errcode') != 0:
                logger.error(_("企业微信消息发送失败! webhook_url:{post_url}, errmsg:{response}").format(post_url=post_url, response=response))
            else:
                logger.info(_("企业微信消息{}发送成功! webhook_url:{post_url}").format(_('分块') if chunk_num else '', post_url=post_url))

        except Exception as e:
            logger.error(_("企业微信消息{}发送失败! {e}").format(_('分块') if chunk_num else '', e=e))
    ...
    def _build_message(self, content, title, msg_type, is_at_all):
        ...
        if msg_type == 'text':
            ...
            raise ValueError(_("不支持的消息类型: {msg_type}").format(msg_type=msg_type))
